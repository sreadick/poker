...
// add betting arrow buttons
	...
	$("#next-hand, #yes").on("click", function() {
		game.round++;
		if (user.betHistory.betPreFlop === true || user.betHistory.betFlop  === true|| user.betHistory.betTurn === true || user.betHistory.betRiver === true) {
			if (user.betHistory.consecRoundBets <= 1) {
				user.betHistory.consecRoundBets++;
			}
		} else {
			user.betHistory.consecRoundBets -= 1;
		}
		...
		if (game.round >= 5) {
			if (user.betHistory.consecRoundBets >= 4) {
		...
		// if (user.betHistory.totalBetsInFiveHands > 10) {
		// 	dealer.playStyle = "aggressive"
		// } else {
		// 	dealer.playStyle = "normal"
		// }

		user.betHistory.betPreFlop = "false";
		user.betHistory.betFlop = "false";
		user.betHistory.betTurn =  "false";
		user.betHistory.betRiver = "false";
    	...
    	newGame(game.userTurn, allInBlind, game.round);
	...
	function Player(playerName) {
		...
		this.bet = function(betAmount) {
			...
			if (this.playerName !== "dealer") {
				...
				// user has 1500
				// user bets 1000... user.currentBet = 1000
				// dealer raises 600... betAmount = 600
		...
		this.fold = function () {
				...
				for (var i = 0; i < cards.length; ++i) {
					...
					for (var j = i+1; j < cards.length; ++j) {
						if (cardSuits[i] === cardSuits[j]) {
							if (progressingFlush.length <= 4) {
								...
								var flushNumbers = [];
								for (var s = 0; s < progressingFlush.length; ++s) {
									flushNumbers.push(progressingFlush[s].numValue);
								}
					...
					if (currentFlush !== undefined && currentFlush.length >= 4) {
						...
						for (var x = 0; x < currentFlush.length; ++x) {
							flushNumbers2.push(currentFlush[x].numValue);
						}
						for (var t = 0; t < currentFlush.length; ++t)  {
							var highCardIndex = flushNumbers2.indexOf(Math.max(...flushNumbers2));
							orderedFlush.push(currentFlush[highCardIndex]);
							flushNumbers2[highCardIndex] = -Infinity;
						}
					...
					if (totalCards[i].numValue !== lowCard) {
						...
						while(count < totalCards.length) {
							...
							// if these conditions have been met there is some type of straight draw
	 							...
	 							var gapIndex = currentStraight.indexOf("gap");
	 							if (currentStraight.contains("gap")) {
									currentStraight.splice(gapIndex, 1);
								}
			...
			if (draw) {
				...
				if (hasStraightFlush) {
					return [9, straightFlush, " has a " + straightFlush[4].value + " high straight-flush"];
				} else if (hasQuads) {
	...
	function dealerTurn() {
		...
		const randomInt =  20 * Math.floor(Math.random() * 21);
		const draw = true;
		var drawInfo;
		var dealerHandInfo;
		var communityFlopFlush = false;
		...
		$("#bet-options *").css("visibility", "hidden");
			...
			if (game.turn === "preFlop" && user.currentBet === 0) {
				...
				// dealer.hand[0] = {value: "ace", numValue: 14, suit: "space", image: "image"};
				// dealer.hand[1] = {value: "two", numValue: 2, suit: "spade", image: "image"};
				...
				// hand contains an ace
					...
					if (dealer.playStyle === "aggressive" || Math.floor(Math.random() * 5) > 2) {
						dealer.bet(300 + randomInt);
					} else {
						dealer.check();
					}
				// user bet pre-flop the last two hands
				} else if (user.betHistory.consecutivePreFlopBets >= 2 && Math.floor(Math.random() * 2) === 0) {
					dealer.bet(300 + randomInt);
				//  dealer is aggressive and has more than 3000 chips than user, bluff one out of four times
				} else if (dealer.playStyle === "aggressive" && dealer.bankroll >= user.bankroll + 3000 && Math.floor(Math.random() * 4) === 0) {
					dealer.bet(300 + randomInt);
				} else {
					dealer.check();
				}
			...
			// Turn is pre-flop and the dealer is small blind or the user has bet
				...
				// dealer.hand[0] = {value: "five", numValue: 5, suit: "space", image: "image"};
				// dealer.hand[1] = {value: "five", numValue: 5, suit: "spade", image: "image"};
				if ((game.pot === 750 && user.currentBet === 250) || (user.currentBet <= 250 && user.bankroll === 0)) {
					...
					// hand contains an ace
						...
						if (dealer.hand[0].numValue > 6 || dealer.hand[1].numValue > 6 && (dealer.playStyle === "aggressive" || Math.floor(Math.random() * 2) === 0))
							dealer.bet(350 + randomInt);
			...
			// turn is flop and dealer is big blind and user hasn't bet or dealer is small blind
				...
				// dealer.hand[1] = {value: "two", numValue: 5, suit: "spade", image: "image"};
				// game.cards[0] = {value: "eight", numValue: 4, suit: "spade", image: "image"};
				// game.cards[1] = {value: "queen", numValue: 7, suit: "spade", image: "image"};
				// game.cards[2] = {value: "jack", numValue: 6, suit: "spade", image: "image"};

				drawInfo = dealer.evalHand(draw);


				straightFlushDraw = false;
				straightAndFlushDraw = false;
				...
				dealerHandInfo = dealer.evalHand();
				...
				// three of a kind
					...
					// hand contains at least one of the trips
					if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
						if (((Math.floor(Math.random() * 3) === 0  && game.userTurn === "first") || (Math.floor(Math.random() * 3) >= 0 && game.userTurn === "second")) || dealer.playStyle === "aggressive") {
					...
					// hand does not contain an one of the trips but hand contains an ace
						...
						// bluff
						if ((dealer.playStyle === "aggressive" && Math.floor(Math.random() * 2) === 0) || Math.floor(Math.random() * 3) === 0) {
							dealer.bet(500 + randomInt);
						} else {
							dealer.check();
						}
				...
				// two pair
					...
					if ((dealerHandInfo[1][0].numValue === dealer.hand[0].numValue || dealerHandInfo[1][1].numValue === dealer.hand[0].numValue) && (dealerHandInfo[1][0].numValue === dealer.hand[1].numValue || dealerHandInfo[1][1].numValue === dealer.hand[1].numValue)) {
						if ((Math.floor(Math.random() * 4) > 0  && game.userTurn === "first") || game.userTurn || dealer.playStyle === "aggressive"){
							dealer.bet(500 + randomInt);
						// slow play
						} else {
							dealer.check();
					...
					//  hand contains one pair with a community card
						...
						if ((Math.floor(Math.random() * 5) >= 0  && game.userTurn === "first") || game.userTurn === "second" || dealer.playStyle === "aggressive") {
							dealer.bet(400 + randomInt);
						} else {
							dealer.check();
						}
					...
					// pocket pair higher than the kicker ("non pair") on the board and higher than 6
						...
						// buy out the pot before user catches something
						if (dealer.playStyle === "aggressive" || Math.floor(Math.random() * 3) === 0) {
							dealer.bet(400 + randomInt);
						} else {
							dealer.check();
				...
				// pair
					...
					if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
						// top pair or pocket pair higher than highest community card
						...
						// some pair (pocket or otherwise) equal to or higher than middle community card
							...
							// kicker is ace or king
							if (((dealer.hand[0].numValue !== dealerHandInfo[1].numValue && dealer.hand[0].numValue >= 13) || (dealer.hand[1].numValue !== dealerHandInfo[1].numValue && dealer.hand[1].numValue >= 13)) || (dealer.playStyle === "aggressive")) {
							...
							if (game.userTurn === "first") {
								// pair is lower than a four
								if (dealerHandInfo[1].numValue <= 4 && dealer.playStyle !== "aggressive") {
									dealer.check();
					...
					// community card pair
						...
						// user has checked and user has bet preflop
						if (game.userTurn === "first" && game.pot > 1000) {
							// both of dealer's cards are higher than a jack
							if (dealer.hand[0].numValue > 11 && dealer.hand[1].numValue > 11 && Math.floor(Math.random() * 4) === 0) {
								dealer.bet(350 + randomInt);
				...
				// straight-flush draw or straight and flush draw
					...
					// user is small blind (and therefore checked) and dealer is aggressive and both of his cards are higher than a 10
					if (game.userTurn === "first" && dealer.playStyle === "aggressive" && (dealer.hand[0].numValue > 10 && dealer.hand[1].numValue > 10)) {
						dealer.bet(300 + randomInt);
					} else if (game.userTurn === "first" && game.pot > 1000) {
						// both of dealer's cards are higher than a jack
						if (dealer.hand[0].numValue > 11 && dealer.hand[1].numValue > 11 && Math.floor(Math.random() * 4) === 0) {
							dealer.bet(350 + randomInt);
						} else if (Math.floor(Math.random() * 5 > 1) && (dealer.hand[0].value === "ace" || dealer.hand[1].value === "ace")) {
			...
			// dealer is small or big blind and user has bet
				...
				if (game.cards[0].suit === game.cards[1].suit && game.cards[1].suit === game.cards[2].suit) {
					communityFlopFlush = true;
				}
				...
				if (dealerHandInfo[0] === 6 || dealerHandInfo[0] === 5) {
					...
					if (Math.floor() * 2 === 0 || dealer.playStyle === "aggressive") {
						dealer.bet(500 + randomInt);
					} else {
						dealer.call();
					}
				// slow play on straight flush, four of a kind, or full house,
					...
					if (Math.floor() * 3 === 0 || dealer.playStyle === "aggressive") {
						dealer.bet(user.currentBet + randomInt);
					} else {
						dealer.call();
					}
				// three of a kind
				} else if (dealerHandInfo[0] === 4) {
					// dealer's hand contains two of the trips
					if (dealer.hand[0].numValue === dealerHandInfo[1].numValue && dealer.hand[0].numValue === dealer.hand[1].numValue) {
						// three community cards of the same suit and user bets more than 3000 and dealer is not not aggressive
						if (communityFlopFlush && user.currentBet <= 3000 && dealer.playStyle !== "aggressive") {
							dealer.fold();
					...
					// dealer's hand contains one of the trips
					} else if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
						// kicker is jack or higher
						if (((dealer.hand[0].numValue !== dealerHandInfo[1].numValue && dealer.hand[0].numValue >= 11) || (dealer.hand[1].numValue !== dealerHandInfo[1].numValue && dealer.hand[1].numValue >= 11))  || dealer.playStyle === "aggressive") {
							dealer.bet(400 + randomInt);
						} else {
							dealer.call();
						}
					// all trips are community cards but dealer has an ace or two face cards
						...
						if (dealer.hand[0].value === "ace" || dealer.hand[1].value === "ace" || (dealer.hand[0].numValue >= 11  && dealer.hand[1].numValue >= 11)) {
				...
				// two pair
					...
					if ((dealerHandInfo[1][0].numValue === dealer.hand[0].numValue || dealerHandInfo[1][1].numValue === dealer.hand[0].numValue) && (dealerHandInfo[1][0].numValue === dealer.hand[1].numValue || dealerHandInfo[1][1].numValue === dealer.hand[1].numValue)) {
						if (communityFlopFlush && user.currentBet <= 3000 && dealer.playStyle !== "aggressive") {
							dealer.fold();
						} else if (dealer.playStyle === "aggressive" || Math.floor(Math.random() * 3)) {
							dealer.bet(400 + randomInt);
						} else {
							dealer.call();
						}
					...
					// pocket pair higher than the highest community card
						...
						if (user.currentBet <= 2000 && (dealer.playStyle === "aggressive" || Math.floor(Math.random() * 3) === 0)) {
							dealer.bet(user.currentBet);
						} else {
							dealer.call();
						}
				...
				// pair
					...
					// not a community card pair and community cards are not the same suit
					if ((dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) && (communityFlopFlush === false && user.currentFlush <= 3000)) {
						...
						if (dealerHandInfo[1].numValue > Math.max(game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue)) {
							...
							} else if (user.currentBet >= 4000 && dealer.playStyle !== "aggressive") {
								dealer.fold();
							} else {
								dealer.call();
							}
						// top pair
							...
							if ((dealer.hand[0].numValue !== dealerHandInfo[1].numValue && dealer.hand[0].numValue >= 12) || dealer.hand[1].numValue !== dealerHandInfo[1].numValue && dealer.hand[1].numValue >= 12) {
								if (Math.floor(Math.random() * 5) > 1 || dealer.playStyle === "aggressive") {
									dealer.bet(300 + randomInt);
								} else {
									dealer.call();
								}
						...
						// lowest pair (or pockets between lowest and middle)
							...
							if (straightFlushDraw || straightAndFlushDraw) {
								...
								dealer.call();
							} else if (user.currentBet <= 1000 && (insideStraightDraw && Math.floor(Math.random() * 2) === 0)) {
								dealer.call();
							} else {
								dealer.fold();
							}
					...
					// community card pair
						...
						if (straightFlushDraw || straightAndFlushDraw) {
							dealer.call();
						} else if (user.currentBet <= 2000 && (flushDraw || outsideStraightDraw) && Math.floor(Math.random() * 3) >= 0) {
							dealer.call();
						} else if (user.currentBet <= 1000 && (insideStraightDraw && Math.floor(Math.random() * 2) === 0)) {
							dealer.call();
						} else {
			...
			// turn is the turn and user hasn't bet
			} else if (game.turn === "turn" && user.currentBet === 0) {

				// dealer.hand[0] = {value: "five", numValue: 13, suit: "diamond", image: "image"};
				// dealer.hand[1] = {value: "two", numValue: 5, suit: "spade", image: "image"};
				// game.cards[0] = {value: "eight", numValue: 5, suit: "heart", image: "image"};
				// game.cards[1] = {value: "queen", numValue: 6, suit: "heart", image: "image"};
				// game.cards[2] = {value: "jack", numValue: 4, suit: "club", image: "image"};
				// game.cards[3] = {value: "king", numValue: 7, suit: "diamond", image: "image"};
				...
				if (drawInfo.cards.contains(dealer.hand[0]) === false && drawInfo.cards.contains(dealer.hand[1]) === false) {
					if (drawInfo.drawType === "straight flush") {
						...
						communityInsideStraightDraw = true;
					} else {
				...
				if (dealerHandInfo[0] === 9 || dealerHandInfo[0] === 8 || dealerHandInfo[0] === 7) {
					// dealer goes first or user bet on the pre-flop or the flop
					if (game.userTurn === "second" || user.betHistory.betPreFlop === true || user.betHistory.betFlop === true) {
						dealer.bet(1100 + randomInt);
					} else if (Math.floor(Math.floor() * 3) >= 0) {
						dealer.bet(600 + randomInt);
					} else {
						dealer.check();
					}
				// flush
					...
					// dealer has two cards making up the flush
					...
					// four cards of the same suit are on the board
					// the flush card in the dealers hand is a queen or higher
						...
						dealer.bet(700 + randomInt);
					// flush card is nine or higher
					} else if (dealer.playStyle === "aggressive" || ((dealerHandInfo[1].contains(dealer.hand[0]) && dealer.hand[0].numValue > 8) || (dealerHandInfo[1].contains(dealer.hand[1]) && dealer.hand[1].numValue > 8))) {
						dealer.bet(500 + randomInt);
					// card is eight or lower and user has checked
				...
				// straight
					...
					// no flush or better draws on the board
					if (communityFlushDraw === false && communityStraightAndFlushDraw === false && communityStraightFlushDraw === false) {
						// no straight draws on the board
						if (communityInsideStraightDraw === false || communityOutsideStraightDraw === false) {
							if (game.userTurn === "first") {
								dealer.bet(800 + randomInt);
							} else {
								dealer.bet(400 + randomInt);
							}
						// there is an outside straight draw on the board and dealer has the higher straight
						} else if (communityOutsideStraightDraw && (dealer.hand[0].numValue === dealerHandInfo[1][4].numValue || dealer.hand[1].numValue === dealerHandInfo[1][4].numValue)) {
							dealer.bet(700 + randomInt);
						// dealer has the lower straight
						} else if (communityOutsideStraightDraw) {
							dealer.bet(500 + randomInt);
						// inside straight on the board
						} else {
							dealer.bet(600 + randomInt);
						}
					// flush or better draws on the board
					} else {
						// user checks
						if (game.userTurn === "first" && Math.floor(Math.random() * 2) === 0) {
							dealer.bet(500 + randomInt);
						} else {
							dealer.check();
						}
				...
				// three of a kind
					...
					// no draws are on the board
					if (communityDraws === false) {
						// hand contains two trips
						if (dealer.hand[0].numValue === dealerHandInfo[1].numValue && dealer.hand[0].numValue === dealer.hand[1].numValue) {
							dealer.bet(650 + randomInt);
						// hand contains one of the trips
						} else if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
							dealer.bet(550 + randomInt);
						// hand does not contain any of the trips
						} else {
							// hand contains an ace
							if (((dealer.hand[0].numValue === "ace" || dealer.hand[1].numValue === "ace") && game.userTurn === "first") || dealer.playStyle === "aggressive") {
								dealer.bet(500 + randomInt);
							} else {
								dealer.check();
							}
						}
					// there are draws on the board
					} else {
						// user is small blind
						if (game.userTurn === "first" || dealer.playStyle === 'aggressive') {
							dealer.bet(450 + randomInt);
						} else {
							dealer.check();
						}
				...
				// two pair
					...
					// dealers hand contains at least one of the pairs
					if (dealer.hand[0].numValue === dealerHandInfo[1][0].numValue || dealer.hand[0].numValue === dealerHandInfo[1][1].numValue || dealer.hand[1].numValue === dealerHandInfo[1][0].numValue || dealer.hand[1].numValue === dealerHandInfo[1][1].numValue) {
						// no draws on the board
						if (communityDraws === false) {
							//  each hand card make up a pair with a community card
							if ((dealerHandInfo[1][0].numValue === dealer.hand[0].numValue || dealerHandInfo[1][1].numValue === dealer.hand[0].numValue) && (dealerHandInfo[1][0].numValue === dealer.hand[1].numValue || dealerHandInfo[1][1].numValue === dealer.hand[1].numValue)) {
								dealer.bet(600 + randomInt);
							//  hand contains one pair with a community card
								...
								game.cards.forEach(function(card) {
									if ((card.numValue !== dealerHandInfo[1][0].numValue || card.numValue !== dealerHandInfo[1][1].numValue) && card.numValue > highCardValue) {
										highCardValue = card.numValue;
									}
								});
								// hand pair is higher than the non pair on the board
								if ((((dealer.hand[0].numValue === dealerHandInfo[1][0].numValue || dealer.hand[0].numValue === dealerHandInfo[1][1].numValue) && dealer.hand[0].numValue === highCardValue) || (dealer.hand[0].numValue === dealerHandInfo[1][0].numValue || dealer.hand[0].numValue === dealerHandInfo[1][1].numValue) && dealer.hand[0].numValue === highCardValue)) {
									dealer.bet(600 + randomInt);
								} else {
									if (game.userTurn === "first" || dealer.playStyle === "aggressive" || Math.floor(Math.random() * 3) > 0) {
										dealer.bet(400 + randomInt);
									} else {
										dealer.check();
									}
							...
							// pocket pair higher than the highest community card
								...
								dealer.bet(600 + randomInt);
							// pocket pair higher than the kicker ("non pair") on the board and higher than 6
							} else if (dealer.hand[0].numValue > median([game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue, game.cards[3].numValue])) {
								if (game.userTurn === "first" || dealer.playStyle === "aggressive" || Math.floor(Math.random() * 2) === 0) {
									dealer.bet(500 + randomInt);
								} else {
									dealer.check();
								}
							// low pocket pair
								...
								if (game.userTurn === "first" || dealer.playStyle === "aggressive" || Math.floor(Math.random() * 4) === 0) {
									dealer.bet(400 + randomInt);
								} else {
									dealer.check();
								}
							}
						// board contains draws
						} else {
							if (game.userTurn === "first" || dealer.playStyle === 'aggressive') {
								dealer.bet(450 + randomInt);
							} else {
					...
					// dealer's hand does not contain either pair
					} else {
						dealer.check();
					}
				// pair
					...
					if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
						...
						if (dealerHandInfo[1].numValue >= Math.max(game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue, game.cards[3].numValue)) {
							dealer.bet(500 + randomInt);
						// some pair (pocket or otherwise) higher than second lowest community card
						} else if (dealerHandInfo[1].numValue >= median([game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue, game.cards[3]])) {
							// pair is second highest
							if (dealer.hand[0].numValue !== dealer.hand[1].numValue) {
								// kicker is ace or king
								if (((dealer.hand[0].numValue !== dealerHandInfo[1].numValue && dealer.hand[0].numValue >= 13) || (dealer.hand[1].numValue !== dealerHandInfo[1].numValue && dealer.hand[1].numValue >= 13)) || (dealer.playStyle === "aggressive")) {
									dealer.bet(500 + randomInt);
								} else if (dealer.playStyle === "aggressive" || Math.floor(Math.random() * 5) > 1) {
									dealer.bet(400 + randomInt);
								} else {
									dealer.check();
								}
							// pocket pair in between second and third lowest
							} else {
								if (dealer.playStyle === "aggressive" || (game.userTurn === "first" && Math.floor(Math.random() * 3) > 0)) {
									dealer.bet(350 + randomInt);
								} else {
									dealer.check();
								}
					...
					// community card pair
						...
						// user has checked and user has bet preflop
						if (game.userTurn === "first" &&  user.betFlop === false) {
							// bluff
							if ((Math.floor(Math.random() * 8) === 0) || (dealer.playStyle === "aggressive" && Math.floor(Math.random() * 3) === 0)) {
								dealer.bet(550 + randomInt);
							} else {
								dealer.check();
							}
			...
			// turn is river user has checked
			} else if (game.turn === "river" && user.currentBet === 0 && game.userTurn === "first") {
				// dealer.hand[0] = {value: "five", numValue: 13, suit: "diamond", image: "image"};
				// dealer.hand[1] = {value: "six", numValue: 6, suit: "heart", image: "image"};
				// game.cards[0] = {value: "eight", numValue: 8, suit: "heart", image: "image"};
				// game.cards[1] = {value: "two", numValue: 2, suit: "heart", image: "image"};
				// game.cards[2] = {value: "four", numValue: 4, suit: "heart", image: "image"};
				// game.cards[3] = {value: "seven", numValue: 7, suit: "heart", image: "image"};
				// game.cards[4] = {value: "three", numValue: 3, suit: "heart", image: "image"};
				// user.hand[0] = {value: "ace", numValue: 5, suit: "heart", image: "image"};
				// user.hand[1] = {value: "ace", numValue: 14, suit: "club", image: "image"};
				...
				// four of a kind
					...
					if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
						dealer.bet(1300 + randomInt);
					// four of a kind of the board and dealer has an ace
					} else if (dealer.hand[0].value === "ace" || dealer.hand[1].value === "ace") {
						dealer.bet(1000 + randomInt);
					// four of a kind of the board and dealer has queen or king and user checked
					} else if (dealer.hand[0].numValue >= 12 || dealer.hand[1].numValue >= 12) {
						if (Math.floor(Math.random() * 3) > 0) {
							dealer.bet(700);
						} else {
					...
					// bluff
						...
						if (((user.betFlop === false && user.betTurn === false) || (dealer.playStyle === "aggressive") && Math.floor(Math.random() * 3) === 0)) {
							dealer.bet(700 + randomInt);
						} else {
							dealer.check();
						}
				...
				// full house
					...
					// trips on the board
					if (dealerHandInfo[1][0].numValue !== dealer.hand[0].numValue || dealerHandInfo[1][0].numValue !== dealer.hand[1].numValue) {
						// the pair (non trip) is the highest pair on the board or pocket overpair
						if ((dealerHandInfo[1][1].numValue === dealer.hand[0].numValue && dealer.hand[0].numValue >= Math.max(game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue, game.cards[3].numValue, game.cards[4].numValue)) || (dealerHandInfo[1][1].numValue === dealer.hand[1].numValue && dealer.hand[1].numValue >= Math.max(game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue, game.cards[3].numValue, game.cards[4].numValue))) {
							dealer.bet(1000 + randomInt);
						} else {
							dealer.bet(700 + randomInt);
						}

					// dealer's hand contains both trip and pair
					} else if ((dealerHandInfo[1][0].numValue === dealer.hand[0].numValue || dealerHandInfo[1][1].numValue === dealer.hand[0].numValue) && (dealerHandInfo[1][0].numValue === dealer.hand[1].numValue || dealerHandInfo[1][1].numValue === dealer.hand[1].numValue)) {
					...
					// two pair on the board
						...
						var highPairValue = 0;
						game.cards.forEach(function(card) {
							// check the board for the highest pair
							if ((card.numValue === dealerHandInfo[1][0].numValue || card.numValue === dealerHandInfo[1][1].numValue) && card.numValue > highPairValue) {
								highPairValue = card.numValue;
							}
						});
						// has the higher trip
						if (dealer.hand[0].numValue === highPairValue || dealer.hand[1].numValue === highPairValue) {
							dealer.bet(900 + randomInt);
						// has the lower trip
						} else {
							dealer.bet(800 + randomInt);
					...
					// full house on the board
						...
						// bluff
						if ((user.betFlop === false && user.betTurn === false && Math.floor(Math.random() * 3) === 0) || (dealer.playStyle === "aggressive" && Math.floor(Math.random() * 2) === 0) || Math.floor(Math.random() * 4) === 0) {
							dealer.bet(800 + randomInt);
						} else {
							dealer.check();
						}
				...
				// flush
					...
					// flush contains at least one card in dealer's hand
					if (dealerHandInfo[1].contains(dealer.hand[0]) || dealerHandInfo[1].contains(dealer.hand[1])) {
						// there is no four card flush on the board or dealer has an ice high flush
				...
				// straight
					...
					// no flush draw on the board
					if (communityFlushDraw === false && communityStraightAndFlushDraw === false && communityStraightFlushDraw === false) {
						// straight on the board
						if ((dealerHandInfo[1].contains(dealer.hand[0]) === false) && (dealerHandInfo[1].contains(dealer.hand[1]) === false)) {
							// bluff
							if ((user.betFlop === false && user.betTurn === false && Math.floor(Math.random() * 3) === 0) || (dealer.playStyle === "aggressive" && Math.floor(Math.random() * 2) === 0) || Math.floor(Math.random() * 5) === 1) {
								dealer.bet(400 + randomInt);
							} else {
								dealer.check();
							}
						// straight contains both of dealer's card
				...
				// three of a kind
					...
					// hand contains at least one of the trips
					if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
						// queen kicker or higher
						if (dealerHandInfo[1].numValue > 11) {
							dealer.bet(700 + randomInt);
						// eight kicker or higher
						} else if (dealerHandInfo[1].numValue > 7) {
							dealer.bet(600 + randomInt);
						// two kicker or higher
						} else {
							dealer.bet(550 + randomInt);
						}
					// trips on the board, ace kicker
					} else if (dealer.hand[0].value === "ace" || dealer.hand[0].value === "ace") {
						dealer.bet(450 + randomInt);
					// bluff
					} else if ((user.betFlop === false && user.betTurn === false && Math.floor(Math.random() * 3) === 0) || dealer.playStyle === "aggressive") {
						dealer.bet(300 + randomInt);
					} else {
						dealer.check();
					}
				// two of a kind
					...
					// dealer hand contains both pairs
					if ((dealerHandInfo[1][0].numValue === dealer.hand[0].numValue && dealerHandInfo[1][1].numValue === dealer.hand[1].numValue) || (dealerHandInfo[1][1].numValue === dealer.hand[0].numValue && dealerHandInfo[1][0].numValue === dealer.hand[1].numValue)) {
						dealer.bet(700 + randomInt);
					// highest board pair or higher higher
					...
					// sevens or higher
						...
						dealer.bet(400 + randomInt);
					// twos or higher
					} else if ((dealerHandInfo[1][0].numValue === dealer.hand[0].numValue) || (dealerHandInfo[1][0].numValue === dealer.hand[1].numValue) || (dealerHandInfo[1][1].numValue === dealer.hand[0].numValue) || (dealerHandInfo[1][1].numValue === dealer.hand[1].numValue)) {
						dealer.bet(300 + randomInt);
					// hand contains neither pair but has an ace high
					} else if (dealer.hand[0].value === "ace" || dealer.hand[1].value === "ace") {
						dealer.bet(300 + randomInt);
					// bluff
					} else if (((user.betFlop === false && user.betTurn === false) || dealer.playStyle === "aggressive") && Math.floor(Math.random() * 5) > 1) {
						if (communityDraws) {
							dealer.bet(650 + randomInt);
						} else {
							dealer.bet(400 + randomInt);
				...
				// pair
					...
					// dealer's hand contains the pair
					if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
						// high pair
						if (dealerHandInfo[1].numValue >= Math.max(game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue, game.cards[3].numValue, game.cards[4].numValue)) {
							dealer.bet(800 + randomInt);
						// greater than middle pair
						} else if (dealerHandInfo[1].numValue > median([game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue, game.cards[3].numValue, game.cards[4].numValue])) {
							dealer.bet(500 + randomInt);
						// middle or lower
						} else {
							dealer.bet(300 + randomInt);
						}
					// dealer's hand does not contain a pair but contains ace
					} else if (dealer.hand[0].value === "ace" || dealer.hand[0].value === "ace") {
						if ((dealer.playStyle === "aggressive" &&  Math.floor(Math.random() * 2) === 0) || Math.floor(Math.random() * 4) === 0) {
							dealer.bet(400 + randomInt);
						} else {
							dealer.check();
						}
					// bluff
						...
						if (((user.betFlop && user.betTurn === false) || dealer.playStyle === "aggressive") && Math.floor(Math.random() * 5) > 1) {
							if (communityDraws) {
							dealer.bet(650 + randomInt);
							} else {
								dealer.bet(400 + randomInt);
				...
				// high card ace
					...
					if (communityDraws === false) {
						if (user.betTurn === false || dealer.playStyle === "aggressive" || Math.floor(Math.random() * 3) >= 1) {
							dealer.bet(400 + randomInt);
						} else {
							dealer.check();
						}
				...
				// bluff
					...
					if ((dealer.playStyle === "aggressive" && Math.floor(Math.random() * 2) === 0) || (user.betTurn === false && Math.floor(Math.random() * 3) === 0) || Math.floor(Math.random() * 7) === 0 ) {
						if (communityDraws) {
							dealer.bet(650 + randomInt);
						} else {
							dealer.bet(400 + randomInt);
				...
				if (dealerHandInfo[0] === 9) {
					if (dealerHandInfo[1].contains(dealer.hand[0]) || dealerHandInfo[1].contains(dealer.hand[1])) {
						dealer.bet(user.currentBet + (1400 + randomInt));
					} else {
						if (dealer.playStyle === "aggressive") {
							dealer.call();
						} else {
					...
					if (dealerHandInfo[1].numValue === dealer.hand[0].numValue || dealerHandInfo[1].numValue === dealer.hand[1].numValue) {
						dealer.bet(user.currentBet + (1300 + randomInt));
					} else if (dealer.hand[0].value === "ace" || dealer.hand[1].value === "ace") {
						dealer.bet(900 + randomInt);
					} else {
						dealer.call();
					...
					if ((dealerHandInfo[1][0].numValue === dealer.hand[0].numValue || dealerHandInfo[1][1].numValue === dealer.hand[0].numValue) && (dealerHandInfo[1][0].numValue === dealer.hand[1].numValue || dealerHandInfo[1][1].numValue === dealer.hand[1].numValue)) {
						...
						dealer.bet(user.currentBet + (400 + randomInt));
					} else if ((dealerHandInfo[1][1].numValue === dealer.hand[0].numValue || dealerHandInfo[1][1].numValue === dealer.hand[1].numValue) && (dealerHandInfo[1][1].numValue >= Math.max(game.cards[0].numValue, game.cards[1].numValue, game.cards[2].numValue, game.cards[3].numValue, game.cards[4].numValue))) {
						dealer.bet(user.currentBet + (300 + randomInt));
					} else if ((dealerHandInfo[1][1].numValue === dealer.hand[0].numValue || dealerHandInfo[1][1].numValue === dealer.hand[1].numValue) && (dealerHandInfo[1][1].numValue > 10)) {
						dealer.bet(700 + randomInt);
	...
	function endTurn() {
		...
		switch(game.turn) {
			...
			if (userHandInfo[0] > dealerHandInfo[0]) {
				...
				for (var j = 0; j < dealerHandInfo[3].length; ++j) {
					if (userHandInfo[3][j].numValue > dealerHandInfo[3][j].numValue) {
						userWins = true;
						userMessage += " with a " + userHandInfo[3][j].value + " kicker";
						break;
					}
					else if (dealerHandInfo[3][j].numValue > userHandInfo[3][j].numValue) {
						dealerWins = true;
						dealerMessage += " with a " + dealerHandInfo[3][j].value + " kicker";
						break;
					}
				}
	...
	function genChips(stack, amount) {
		...
		for (var j = 0; j < 6; ++j) {
			if (chipType[j] === 0) {
				$(stack + " div:nth-child(" + (j + 1) + ")").hide();
			} else {
				for (var k = 1; k <= chipType[j]; k++) {
					$(stack + " div:nth-child(" + (j + 1) + ")").css("display", "inline-block");
					if (j === 0) {
	...
	function updateLeaderBoard(newPlayer, newScore) {
		...
		$("#high-scores-screen").show("slide", { direction: "up" }, 2000);
